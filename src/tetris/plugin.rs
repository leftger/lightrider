...
pub(crate) fn spawn_tetris_board(
    ...
    for (index, colour) in rendered.iter().copied().enumerate() {
        let col = index % config::arcade::TETRIS_COLS;
        let row = index / config::arcade::TETRIS_COLS;
        let x = (col as f32 - (config::arcade::TETRIS_COLS - 1) as f32 * 0.5) * 1.2;
        // Row 0 is the top of the board, so higher rows sit lower on screen.
        // The whole board is lifted above the arena floor.
        let y = ((config::arcade::TETRIS_ROWS - 1) - row) as f32 * 1.2 + 0.6;
        commands.spawn((
            LightcycleSceneRoot,
            BlockEntity { index },
    ...
    let board_half = config::arcade::TETRIS_COLS as f32 * 0.6;
    let wall_x = board_half + 0.55;
    let board_height = config::arcade::TETRIS_ROWS as f32 * 1.2;
...
pub(crate) fn sync_tetris_entities(
    ...
    for (entity, mut transform, mut visibility, mut material) in &mut blocks {
        match rendered.get(entity.index) {
            Some(Some(colour)) => {
                let col = entity.index % config::arcade::TETRIS_COLS;
                let row = entity.index / config::arcade::TETRIS_COLS;
                transform.translation = Vec3::new(
                    (col as f32 - (config::arcade::TETRIS_COLS - 1) as f32 * 0.5) * 1.2,
                    ((config::arcade::TETRIS_ROWS - 1) - row) as f32 * 1.2 + 0.6,
                    0.0,
                );
